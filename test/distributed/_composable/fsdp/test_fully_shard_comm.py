...
class TestMoriSdmaAllGather(TestCase):
    ...
    def test_zero_copy_output_capability(self):
        self.assertFalse(MoriSdmaAllGather(zero_copy_output=False).supports_param_contiguous_output)
        self.assertTrue(MoriSdmaAllGather(zero_copy_output=True).supports_param_contiguous_output)
    ...
    def test_env_flag_parsing(self):
        ...
        for value, enabled in (("1", True), ("off", False), ("", False)):
            with unittest.mock.patch.dict(
                os.environ, {"MORI_FSDP_ENABLE_SDMA": value}
            ):
