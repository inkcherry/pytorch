...
def is_mori_fsdp_sdma_enabled() -> bool:
    """Whether ``MORI_FSDP_ENABLE_SDMA`` requests the MORI SDMA backend."""
    raw = os.environ.get("MORI_FSDP_ENABLE_SDMA", "").strip().lower()
    return raw not in ("", "0", "false", "no", "off")


def is_mori_fsdp_zero_copy_output_enabled() -> bool:
    """Whether ``MORI_FSDP_ZERO_COPY_OUTPUT`` requests zero-copy output."""
...
class MoriSdmaAllGather(AllGather):
    ...
    Args:
        zero_copy_output (bool | None): produce a parameter-contiguous output
            that FSDP uses in place. Defaults to the
            ``MORI_FSDP_ZERO_COPY_OUTPUT`` environment variable.
    """

    def __init__(self, zero_copy_output: bool | None = None) -> None:
        if zero_copy_output is None:
            zero_copy_output = is_mori_fsdp_zero_copy_output_enabled()
    ...
    def __call__(
        ...
        if self._can_call_param_contiguous(input_tensor):
            ...
            if split_sizes is None or split_offsets is None:
                raise RuntimeError(
                    "MORI param-contiguous all-gather metadata is not initialized"
                )
            if async_op:
    ...
    def prepare_param_contiguous_output(
        ...
        if not all_gather_input_split_sizes:
            raise RuntimeError("MORI zero-copy all-gather requires non-empty splits")
        if sum(all_gather_input_split_sizes) != all_gather_input_numel:
            raise RuntimeError(
                "MORI zero-copy all-gather split sizes do not match input numel"
            )
        # MORI addresses the transfer in 4-byte (uint32) words.
        ...
        for split_size in all_gather_input_split_sizes:
            ...
            if split_nbytes % 4 != 0:
                raise RuntimeError(
                    "MORI zero-copy all-gather requires every split to be "
                    "4-byte aligned"
                )
            split_offsets_u32.append(offset)
            split_sizes_u32.append(split_nbytes // 4)
            offset += split_nbytes // 4
        if offset * 4 != all_gather_input_numel * element_size:
            raise RuntimeError("MORI zero-copy all-gather byte size mismatch")
    ...
    def _validate_tensors(
        ...
        if not input_tensor.is_cuda or not output_tensor.is_cuda:
            raise RuntimeError("MORI FSDP SDMA all-gather requires CUDA tensors")
        if input_tensor.device != output_tensor.device:
            raise RuntimeError(
                "MORI FSDP SDMA all-gather requires input and output on the same device"
            )
        if input_tensor.dtype != output_tensor.dtype:
            raise RuntimeError(
                "MORI FSDP SDMA all-gather requires matching input/output dtypes"
            )
        expected_numel = input_tensor.numel() * group.size()
        if output_tensor.numel() != expected_numel:
            raise RuntimeError(
                "MORI FSDP SDMA all-gather expected output numel "
                f"{expected_numel}, got {output_tensor.numel()}"
            )
        if _tensor_nbytes(input_tensor) % 4 != 0 or _tensor_nbytes(output_tensor) % 4:
            raise RuntimeError(
                "MORI FSDP SDMA all-gather requires 4-byte aligned input/output sizes"
            )
    ...
    def _get_collective(self, group: dist.ProcessGroup) -> Any:
        ...
        if my_pe != rank or npes != world_size:
            raise RuntimeError(
                "MORI FSDP SDMA all-gather requires the FSDP process group to "
    ...
    def _ensure_output_registered(
        ...
        if self._zero_copy_output and not collective.is_output_registered(
            ...
            raise RuntimeError(
                "MORI FSDP SDMA all-gather requires registered output buffers "
