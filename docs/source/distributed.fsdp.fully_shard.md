...
By default the all-gather output uses a rank-major (``[rank][param]``) layout
and FSDP copies each parameter out into its own storage. A backend that can
instead write a parameter-contiguous (``[param][rank]``) output may set
